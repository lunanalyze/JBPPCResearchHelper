...
``JBResearchReportHelper`` 의 자동 업데이트와 **같은 규격**이다. manifest 형식·SHA-256 검증·
별도 프로세스 교체·실패 시 자동 복구가 모두 동일하고, 이 앱에 맞춰 바뀐 것은 이름과 주소뿐이다.

* 실행 파일: ``PPCResearchHelper.exe``
* 배포처: ``lunanalyze/JBPPCResearchHelper`` (릴리스 전용 공개 저장소)
* 포트: 8766
...
* **교체 스크립트는 PowerShell**(``updater/apply.ps1``). 이 앱의 Python 은 교체 대상인 exe 안에
  갇혀 있어 쓸 수 없다. PowerShell 은 Windows 에 항상 있다.
...
#: 인증 없이 읽혀야 한다.
DEFAULT_FEED_URL = (
    "https://github.com/lunanalyze/JBPPCResearchHelper"
    "/releases/latest/download/latest.json"
)

APP_EXE_NAME = "PPCResearchHelper.exe"
UPDATER_SCRIPT = "apply.ps1"
WORK_DIR_NAME = "ppc-research-helper-update"

_USER_AGENT = "ppc-research-helper-updater"
...
def feed_url() -> str:
    return (os.environ.get("PPCRH_UPDATE_FEED") or DEFAULT_FEED_URL).strip()


def enabled() -> bool:
    return (os.environ.get("PPCRH_UPDATE_ENABLED") or "1").strip().lower() not in _FALSY
...
def install_root() -> Path | None:
    ...
    인스톨러가 만드는 구조::
        ...
        <root>\\PPCResearchHelper.exe   ← 지금 실행 중인 이 파일(교체 대상)
...
def is_newer(latest: str, current: str) -> bool:
    """``1.0.9`` vs ``1.0.10`` 처럼 자리수가 다른 경우를 문자열 비교로 틀리지 않게 숫자 단위로 본다.
...
def check() -> dict:
    ...
    **실패는 오류가 아니다** — 망이 막힌 PC 도 있으므로 사유만 담아 돌려준다. 화면이 배너를
    ...
    except Exception as exc:  # 망 문제까지 전부 '확인 못 함'으로 접는다
