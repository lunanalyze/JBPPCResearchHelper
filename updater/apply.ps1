# ============================================================================
# PPC 조사연구 도우미 업데이터 — 앱이 죽은 뒤 exe 를 갈아끼우고 다시 띄운다.
#
...
#   파싱되지 않는다**("The string is missing the terminator"). updater.py 의 spawn() 이
...
# 브라우저는 열지 않는다(PPCRH_NO_BROWSER=1). 업데이트를 시작한 탭이 이미 /heartbeat 를
...
function Start-App {
    try {
        ...
        $psi.EnvironmentVariables["PPCRH_NO_BROWSER"] = "1"
...
if (-not (Wait-Unlocked -Path $ExePath -TimeoutSeconds 180)) {
    ...
    Write-RunningProcesses
    Write-Log "업데이트를 중단합니다 — 열려 있는 'PPC 조사연구 도우미' 를 모두 닫고 다시 시도하세요."
...
if (Test-Healthy) {
    ...
    # 키 이름은 installer.nsi 의 COMPANY_NAME 과 같아야 한다.
    if ($Version) {
        try {
            $key = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\PPCResearchHelper"
