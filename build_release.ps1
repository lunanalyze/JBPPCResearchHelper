...
#   * 팩 안에는 PPCResearchHelper.exe 하나만 들어간다.
#
# 쓰는 법:
...
#
# ⚠ 릴리스 저장소는 **공개**여야 한다. 앱에 토큰을 심지 않으려면 인증 없이 읽혀야 한다.
...
param(
    ...
    [string]$Notes = "",
    # 팩을 내려받을 주소. 기본값은 릴리스 배포용 공개 저장소의 v<버전> 릴리스 에셋.
    [string]$RepoUrl = "https://github.com/lunanalyze/JBPPCResearchHelper"
)
...
$ExePath = Join-Path $Root "dist\PPCResearchHelper.exe"
...
$PackName = "PPCResearchHelper-Update-$Version.zip"
...
Write-Host ""
Write-Host "다음: GitHub 에 v$Version 태그로 릴리스를 만들고 latest.json / $PackName / PPCResearchHelperSetup.exe 를 에셋으로 올리세요."
